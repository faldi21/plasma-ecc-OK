#!/usr/bin/env bash
# scripts/verify_paper1.sh — cek Definition of Done D1..D7 (docs/EXPERIMENT_PRD.md §0)
#
# Layout: repo kode ~/plasma-ecc-OK, repo paper ~/paper1 (sejajar, tidak nested).
#
# Pakai:
#   make -f Makefile.paper1 p1-verify
# atau:
#   RUN_ID=20260920-141233-a3f19c2 bash scripts/verify_paper1.sh
#
...
if [[ -z "$RUN_ID" ]]; then
  echo "RUN_ID belum diset dan $DATA_ROOT/LATEST tidak ada. Jalankan: make -f Makefile.paper1 p1-freeze" >&2
  exit 2
fi
...
printf '\n\033[1mverify_paper1\033[0m  RUN_ID=%s\n  paper: %s\n  data : %s\n' "$RUN_ID" "$PAPER_TEX" "$RAW"
...
if git rev-parse "$FREEZE_TAG" >/dev/null 2>&1; then
  ...
  bad "tag $FREEZE_TAG belum dibuat (jalankan: make -f Makefile.paper1 p1-freeze)"
fi
