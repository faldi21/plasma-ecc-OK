...
/**
 * @title PlasmaChainUTXOMerkleInline
 * @dev E3 "inline" placement cell for the Merkle primitive (docs/
 * EXPERIMENT_PRD.md section 6.1, docs/TICKETS.md T7): identical to
 * PlasmaChainUTXOMerkle.sol EXCEPT accumulator.add() is called immediately
 * whenever ANY new UTXO is created -- deposits AND transfer outputs alike
 * (inside createDepositUtxo/createDepositUtxoBatch and _executeBatchOp /
 * _createOutputs) -- instead of being deferred to createBlock/
 * createBlockChunked. Mirrors PlasmaChainUTXOInline.sol's fork of
 * PlasmaChainUTXO.sol -- see that file's docblock for the full rationale
 * (git-history precedent for "inline" as the original design, why
 * createBlock/createBlockChunked are left unmodified and harmless thanks
 * to MerkleAccumulator.add's duplicate guard, and why deposits must also
 * be inline here -- not just transfers -- to preserve the same element
 * insertion ORDER as the deferred contract: an incremental Merkle tree's
 * root depends on insertion order, unlike ASC's additive accumulator, so
 * leaving deposits deferred while transfers went inline reversed the
 * deposit-vs-transfer insertion order relative to PlasmaChainUTXOMerkle.sol
 * and broke digest parity at n>1 -- caught by contracts/test/
 * E3PlacementParity.t.sol, not assumed).
 *
 * BENCHMARK BASELINE — identical to PlasmaChainUTXOInline except the
 * cryptographic primitive backing the accumulator: this contract uses an
 * incremental Merkle tree (OpenZeppelin), while PlasmaChainUTXOInline uses
 * the ECC accumulator library based on elliptic curve cryptography.
 *
 * Key API differences from PlasmaChainUTXOInline:
 ...
 */
contract PlasmaChainUTXOMerkleInline {
