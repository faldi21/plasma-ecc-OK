// Verbatim copy of contracts/src/PlasmaChainUTXO.sol at commit 9fdd117,
// vendored so that one frozen tag can measure both the as-submitted and the
// memory-optimized implementation. Do not edit.
//
// Exactly two lines differ from the original (everything else below this
// header is byte-identical to git show 9fdd117:contracts/src/PlasmaChainUTXO.sol):
//   - the import path, now pointing at the co-located ECCAccumulatorV0.sol
//     instead of ../libraries/ECCAccumulator.sol
//   - the contract name, PlasmaChainUTXO -> PlasmaChainUTXOV0, required to
//     avoid a duplicate-declaration compile error against the current
//     contracts/src/PlasmaChainUTXO.sol
// Every internal reference to `ECCAccumulator.*` in the body below is
// UNCHANGED, because ECCAccumulatorV0.sol's library declaration itself
// keeps the name `ECCAccumulator` (see its own header) -- the import above
// binds that same symbol name locally to this file.
//
// Verify: diff the two changed lines above against
//   git show 9fdd117:contracts/src/PlasmaChainUTXO.sol

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.30;

import "./ECCAccumulatorV0.sol";
