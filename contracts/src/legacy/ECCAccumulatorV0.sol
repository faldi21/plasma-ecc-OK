// Verbatim copy of contracts/src/libraries/ECCAccumulator.sol at commit
// 9fdd117, vendored so that one frozen tag can measure both the
// as-submitted and the memory-optimized implementation. Do not edit.
//
// The library declaration below is kept as `library ECCAccumulator` --
// unchanged from the original -- to preserve byte-for-byte equivalence of
// everything after this header. PlasmaChainUTXOV0.sol imports the
// `ECCAccumulator` symbol from THIS file specifically via its own import
// statement; Solidity import bindings are scoped per importing file, not
// globally unique across a project, so this does not collide with the
// current contracts/src/libraries/ECCAccumulator.sol (which
// PlasmaChainUTXO.sol imports separately, unrelated to this file).
//
// Verify byte-identity of everything below this header with:
//   git show 9fdd117:contracts/src/libraries/ECCAccumulator.sol | diff - <(tail -n +17 contracts/src/legacy/ECCAccumulatorV0.sol)
